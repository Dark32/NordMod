...
//        if (player.getHeldItem() != null && player.getHeldItem().getItem() == NordCoreItems.debugStickItemSchematickSave) {
        if (player.getHeldItem() != null && player.getHeldItem().getItem() instanceof ISelectItem) {
            ItemStack itemStack = player.getHeldItem();
            ((ISelectItem)itemStack.getItem()).renderSelectArea(itemStack, player,event.partialTicks);
